...
class NotificationController {
  ...
  static Future<void> scheduleDailyNotifications() async {
    ...
    if (!isAllowed) return;
     ...
     final List<Map<String, int>> scheduleList = [];
  int hour = 8; // Mulai dari jam 8 pagi
  int minute = 0;

  while (hour <= 22) { // Berhenti di jam 10 malam (22:00)
    scheduleList.add({'hour': hour, 'minute': minute});

    // Algorithm penambahan 1,5 jam (90 menit)
    hour += 1; // Tambah 1 jam
    minute += 30; // Tambah 30 menit

    // Jika menit >= 60, sesuaikan jam dan menit
    if (minute >= 60) {
      hour += 1;
      minute -= 60;
    }
  ...
  // Buat notifikasi untuk setiap jadwal
  for (int i = 0; i < scheduleList.length; i++) {
    await AwesomeNotifications().createNotification(
      content: NotificationContent(
        ...
        body: 'Yuk minum dulu biar tetap sehat dan segar 💧',
  ...
  /// Schedule a notification in seconds
  static Future<void> myNotifyScheduleInSeconds({
    ...
    var nowDate = DateTime.now().add(Duration(seconds: secondsFromNow));
    await AwesomeNotifications().createNotification(
      schedule: NotificationCalendar(
        year: nowDate.year,
        month: nowDate.month,
      ...
      body: Center(
        child: Column(
          ...
          children: const <Widget>[
            Text(
              'Push the button below to schedule daily hydration reminders',
            ),
          ],
        ),
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () => NotificationController.scheduleDailyNotifications(),
        tooltip: 'Schedule Daily Reminders',
