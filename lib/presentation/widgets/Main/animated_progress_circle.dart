...
void _updateAnimationPlaybackBasedOnState() {
  ...
  ProgressState _getProgressState() {
    ...
    if (ratio > 1.0) return ProgressState.exceeded; // Lebih dari 100% hingga 200%
 ...
 Color _getPercentageTextColor(ProgressState state) {
  ...
  Color _getCurrentIntakeValueColor(ProgressState state) {
    if (state == ProgressState.empty && widget.currentIntake <=0) return Colors.transparent;
    if (state == ProgressState.critical) {
      return Color.fromARGB(255, 228, 128, 70); // Warna lebih gelap untuk critical
    }
    if (state == ProgressState.exceeded) {
      return Color(0xFF003D7A);
    }
...
class ProgressArcPainter extends CustomPainter {
    ...
    if (progressState != ProgressState.empty && progress > 0) {
      ...
      switch (progressState) {
        ...
        case ProgressState.exceeded: // Progress > 100% and <= 200%
          ...
          double orangeSweepPercent = (actualAnimatedProgress - 100.0).clamp(0.0, 100.0);
          _drawProgressArc(
            canvas, 
            rect, 
            startAngleRad, 
            ...
            center,
            radius,
            orangeSweepPercent < 100.0 // Show seek indicator only if not complete
          );
          break;
        ...
        default:
          ...
          _drawProgressArc(
            ...
            center,
            radius,
            blueSweepPercent < 100.0 // Show seek indicator only if not complete
          );
          break;
      }
    }
  ...
  void _drawProgressArc(
    ...
    final currentSweepRad = (sweepAngleDegrees * sweepPercent / 100) * (math.pi / 180);
